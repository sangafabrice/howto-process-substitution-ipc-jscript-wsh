...
& {
  ...
  $mainScriptPath = 'cvmd2html.js'
  $infoFilePath = "$Root\$mainScriptPath"
  ...
  if ($null -ne $diffLines) {
    ...
    $version = ($diff -eq 0 ? 1:$diff) + ([version](git cat-file -p HEAD:$mainScriptPath | Where-Object { $_ -match $matchRegex } | Select-Object -Last 1 | ForEach-Object { [void]($_ -match $matchRegex); $Matches.Version })).Revision
